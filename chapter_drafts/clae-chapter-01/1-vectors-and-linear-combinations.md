...
One more thing before the data, because it decides whether you can trust your own drawings. Two vectors span at most a plane. That stays true in three dimensions, in 1,460, in a googol: the reach of the operation is bounded by the number of ingredients, never by the size of the space the ingredients live in. Nobody can picture 1,460 dimensions, and nobody needs to. Every question we ask about two vectors happens inside the at-most-a-plane they span, so the drawing on this page is exact for the high-dimensional case, no metaphor involved. Standard courses observe that two vectors in three-dimensional space span a plane. The better sentence: two vectors in 1,460-dimensional space still span only a plane. You were never supposed to see the ambient space.
