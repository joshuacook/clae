<!-- PREFACE v6 (2026-07-13): the ruled census pass (verified census
     chapter_notes/preface-v5-ink-verified.md + the double-census rulings).
     Beyond the census edits (deletes, colon purge, unhorsed, 3x3 shown,
     three ways shown, TikZ projection, uniqueness moved to Ch 1, Singer to
     footnote, honesty beat, Caltech+UCLA provenance): ruling 1 adds the
...
Every idea in this book is seen through more than one lens, and the lenses have names. Through the geometric lens a vector is an arrow, and scaling it is a stretch. Through the algebraic lens the same vector is a short list of numbers, worked with a pencil. Through the computational lens it is an array with a clock running. Through the data lens it is a column of house prices with a question attached.

First-year algebra teachers call this practice multiple representations. A function is a plot, a formula, a table, and a pattern, and the student who can move among the four owns the function. It is pedagogical genius. This book runs on it.

When the book switches lenses, the margin says so. A small tag sits at each switch, geometric, algebraic, computational, or data, so you always know which way of looking is doing the work. No idea is required to visit all four. The book reaches for whichever lens shows the idea best.
...
Strang's chapter 4 is orthogonality, and there is a drawing in it. A vector hangs just off a subspace. A perpendicular drops to the nearest point. The whole machinery of regression falls out of that one picture.
...
That is the picture that converted me. This book has one destination, and it is that drawing. You will meet it in Chapter 11.
