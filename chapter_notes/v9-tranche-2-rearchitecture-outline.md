...
- **2.6 The shadow, whole (#69, #72, #73).** Projection gets the freed room and a complete arc with no imports: the picture (noon sun), the pencil (score, calibrate, stretch), **the transpose used without pausing (Shaw's #71 ruling: it lives in its own interlude after the product section, not here)**, the projection matrix via the outer product of 2.3, properties measured, the at-scale drawing, and the Ch 12 promise. Nothing arrives out of nowhere: dot product from the preface, outer product from 2.3, transpose introduced in place.
