<!-- CHAPTER 5 DRAFT V1 (2026-07-19): NET NEW for the v11 pass per the
     v10 ink census (census 2: the houses move here; "draft chapter 5
     right now"). Part II opens: the dataset arrives, the matrix's data
     identity (from ch2 V6 stock), probability spaces and random
     variables built on Chapter 1's vector-as-function reading, columns
     as realizations, standardization (from ch2 stock), and the door
     (from ch3 stock): existence fails on real evidence. Write-from-
     scratch probability core + adapted stock blocks. -->
