...
def replace_empty(df, feature, value):
    new_categories = df[feature].cat.categories.tolist()
    new_categories.append(value)
    df[feature] = df[feature].cat.set_categories(new_categories)
    df.loc[df[feature] == '', feature] = value
    return df[feature]

for feature in empty_means_without:
    replace_empty(housing_df, feature, "None")

# Comment out dropna as we've already handled the missing values above
...
def feature_outliers(dataframe, col, param=1.5):
    ...
    # drop the nulls for the feature
    try:
        feature_no_nulls = dataframe[col].dropna()
        Q1 = np.percentile(feature_no_nulls, 25)
        Q3 = np.percentile(feature_no_nulls, 75)
...
numeric_log_std_sc_out_rem_df = numeric_log_std_sc_df.copy() #drop(multiple_outliers(numeric_log_std_sc_df, 5))
categorical_encoded_out_rem_df = categorical_encoded_df.copy() #drop(multiple_outliers(numeric_log_std_sc_df, 5))
target_log_std_sc_out_rem_df = target.copy() # drop(multiple_outliers(numeric_log_std_sc_df, 5))
...
# Don't delete Counter as it's used in multiple_outliers
...
del feature
# Don't delete feature_outliers as it's used in multiple_outliers
del housing_df
del listing_df
# Don't delete multiple_outliers as it might be used elsewhere
del replace_empty
del sale_df
del st
del stats
